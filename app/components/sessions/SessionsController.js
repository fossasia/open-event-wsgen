        ...
        ['$mdDialog', '$sessionStorage', '$rootScope', 'ApiJsonFactory',
            function($mdDialog, $sessionStorage, $rootScope, ApiJsonFactory) {
               ...
               sc.showSession = function(session, event) {
                ...
                };/*Display only when id match */
                sc.display=function(session,event){
                   
                    

                   for(i=0;i<sc.Days.length;i++){
                             
                           for(var x in sc.Days[i].sessions){
                       
                            if(sc.Days[i].sessions[x].id==session){

                               
                                console.log(event);
                                if( document.getElementById(session).style.display =="none"){
                               n=document.getElementById(session);
