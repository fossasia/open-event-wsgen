    ...
    $(document).ready(function () {
        ...
        function resetPage() {
            if (previousRoomHeight > 0) {
                var footer = $(roomsdiv).find('.calendar-footer');
                $(roomsdiv).css({
        ...
        function adjustPage(element) {
            ...
            if (session.is(element)) {
                var rooms = $(element).parent().parent().parent();
                var footer = $(rooms).find('.calendar-footer');
                var roomsPosition = $(rooms).offset();
                var roomsHeight = $(rooms).outerHeight();
                var roomsWidth = $(rooms).outerWidth();
                var pop = $(element).find('.pop-box');
                var popPosition = $(pop).offset();
                ...
                if (openedPop !== null) {
                    var parentWidth = roomsPosition.left + roomsWidth;
                    var childWidth = popPosition.left + popWidth;
                    var parentHeight = roomsPosition.top + roomsHeight;
                    var childHeight = popPosition.top + popHeight;
        ...
        function adjustFooter(event) {
            ...
            var imageHoverheight = imageholder.offset().top;
            var imageContainer = imageholder.outerHeight();
            var popBox = imageholder.next();
            var popBoxheight = popBox.outerHeight();
            var totalHeight = imageHoverheight + imageContainer + popBoxheight;
            if ($('.speakers-row').offset() !== undefined) {
                var speakersRow = $('.speakers-row').offset().top + $('.speakers-row').outerHeight() + $('.classic').outerHeight();
                var _shift = totalHeight - speakersRow;
                if (_shift > 0) {
                    $(".classic").css({
                        "position": "absolute",
                        "top": speakersRow + _shift,
                        "width": "100%",
                        "z-index": "999"
                    });
                }
...
;(function () {
    ...
    var icsGen = function icsGen() {
        ...
        return {
            ...
            'addEvent': function addEvent(session) {
                var calendarEvent = ['BEGIN:VEVENT', 'UID:' + session.uid, 'CLASS:PUBLIC', 'DESCRIPTION:' + session.description, 'DTSTART;VALUE=DATETIME:' + session.begin, 'DTEND;VALUE=DATE:' + session.stop, 'LOCATION:' + session.location, 'SUMMARY;LANGUAGE=en-us:' + session.subject, 'TRANSP:TRANSPARENT', 'END:VEVENT'].join(SEPARATOR);
             ...
             */
            'download': function download(filename, ext, dlh) {
